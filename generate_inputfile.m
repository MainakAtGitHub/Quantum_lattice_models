...
function f=generate_inputfile(oldFolder, Gamma_file,... 
    ...
    Gamma1_file,Gamma2_file,Gamma3_file,tripletOrderFile,Jpr,mu_guess,compl_ini_delta)
        ...
        for itr_kT = kT
            for itr_U = U
                if U_Upr_J_related
                    ...
                    J=itr_U/4; Jpr=J; Upr=itr_U-2*J; 
                end
                if U_Upr_J_related
                    J=itr_U/4;
                end
                cd(oldFolder);
%                 mkdir(['U_',num2str(itr_U)]); cd(['U_',num2str(itr_U)])
                inputfile = [...
                    'input_','.txt'];
%                     num2str(nOrbitals),'band_N','[',num2str(itr_N(1)),'_',num2str(itr_N(2)),']',...
...
%                     '_eff_pot',num2str(eff_pot),...
                ...
                fprintf(infile_id,['N=','[',num2str(itr_N(1)),',',num2str(itr_N(2)),']','\n']);
                ...
                fprintf(infile_id,['casestring=','LDOS_',...
                    ...
                    '_NN_cut_off',num2str(NN_cut_off),...
                    '_eff_pot',num2str(eff_pot),'\n']);
                fprintf(infile_id,'deltaTol=1e-07\n'); % may need frequent change
                ...
                fprintf(infile_id,['J_pr=',num2str(Jpr),'\n']);
...
%                 fprintf(infile_id,'eff_pot=8\n');
                ...
                fprintf(infile_id,['tripletOrderFile=',tripletOrderFile,'\n']);
                fprintf(infile_id,['magTol=','1e-9','\n']);
                fprintf(infile_id,['Orb_seq=[2,3,4,5,1]','\n']);
                fprintf(infile_id,['compl_ini_delta=',num2str(compl_ini_delta)]);
                fclose(infile_id);
                if ~justRun
                    submit_inputfile = [...
                        'submit_input_','.sh'];
...
%                   fprintf(submit_infile_id,['#SBATCH --output=slurm_','\%','N_','\%','j.out     # Standard output and error log\n']);
                    ...
                    fprintf(submit_infile_id,'#SBATCH --qos=hirschfeld-b\n');
%                   fprintf(submit_infile_id,'##SBATCH --partition=hpg2-dev\n');
%                     fprintf(submit_infile_id,['./run_BdG_impurity_v3.sh /apps/matlab/mcr/2019b/v97 ',inputfile]);
                    fprintf(submit_infile_id,['./run_BdG_impurity_v3.sh /apps/matlab/mcr/2020b/v99 ',inputfile]);
...
%               cd(oldFolder);
                if ~justRun
                    cd(BdGfolder);
                    ini_guess_filename=generate_initialguess(itr_N,Gamma_file,n0_orb,mu_guess,...
...
%                 else
                    load(pos_file,'-mat');
                    dislocation_length=itr_N(1)*itr_N(2)-size(r,1);
