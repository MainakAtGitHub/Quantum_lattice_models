function t = hoppings2(r,N_lin,figg,t_r_ref) % r is a :by2 matrix of coordinates
...
%          t(i,near_neighbour_index(j)) = -1;
         %%% NEW REALISTIC HOPPING STRUCTURE
         if isempty(t_r_ref)
             t(i,near_neighbour_index(j)) = -1/exp(-1)*exp(-(d(i,near_neighbour_index(j)))^2);
         else
             d_x(i,near_neighbour_index(j))=min([abs(r(near_neighbour_index(j),1)-r(i,1)),abs(r(near_neighbour_index(j),1)-r(i,1)-N_lin),abs(r(near_neighbour_index(j),1)-r(i,1)+N_lin)]);
             d_y(i,near_neighbour_index(j))=min([abs(r(near_neighbour_index(j),2)-r(i,2)),abs(r(near_neighbour_index(j),2)-r(i,2)-N_lin),abs(r(near_neighbour_index(j),2)-r(i,2)+N_lin)]);
             t(i,near_neighbour_index(j)) = griddata(t_r_ref(:,1),t_r_ref(:,2),(10^(-3))*t_r_ref(:,3),d_x(i,near_neighbour_index(j)),d_y(i,near_neighbour_index(j)),'natural');
         end
