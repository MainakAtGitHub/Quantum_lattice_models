%RINT_EPS  Prints figures to eps with fonts embedded
%
% Examples:
%   print_eps filename
...
% $Id: print_eps.m,v 1.14 2009/01/19 11:01:07 ojw Exp $
...
tmp_nam = tempname;
print('-depsc2', '-noui', '-painters', ['-f' num2str(fig)], '-r864', [tmp_nam '.eps']);
% Fix the line styles
% do not fix the line style as the sqares of Matlab are replaced by smaller
% dots with this routine
...
%cmd = ['pdftops -q -paper match -pagecrop -eps -level2 ' tmp_nam '.pdf "' name '"'];
cmd = ['pdftops -q -paper match -eps -level2 ' tmp_nam '.pdf "' name '"'];
