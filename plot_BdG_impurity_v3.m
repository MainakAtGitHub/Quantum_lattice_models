...
if exist('nUpAcc','var')
    subplot(2,2,3); plot(abs(deltaMaxAcc)); title('max(deltaNN,deltaNNN)'); %axis('square');
    hold on
    subplot(2,2,3); plot(abs(deltaMinAcc)); %axis('square');
    subplot(2,2,4); 
    plot(abs(nUpAcc)); title('max(nUpAcc)'); %axis('square');
else
