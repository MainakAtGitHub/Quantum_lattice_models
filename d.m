...
d1=zeros(1,length(e));
% d2=d1;
fprintf(1,['\nof ',num2str(size(kx,1)-1),' are done:  ']);
for i=1:size(kx,1)-1
%       Printing Progress to the Command Window
       testoutput(i,i-1);   
    tmp=0;
    for j=1:size(ky,2)-1
     E1=[E(i,j);E(i,j+1);E(i+1,j)]; %triangle without point i+1,j+1
     k1=[kx(i,j),ky(i,j);kx(i,j+1),ky(i,j+1);kx(i+1,j),ky(i+1,j)];
     r1=triD(E1,k1,e);
     E1=[E(i+1,j+1);E(i,j+1);E(i+1,j)]; %triangle without point i,j
     k1=[kx(i+1,j+1),ky(i+1,j+1);kx(i,j+1),ky(i,j+1);kx(i+1,j),ky(i+1,j)];
     r2=triD(E1,k1,e);
...
%      r4=triD(E1,k1,e);
     % in order to minimize the errors from the triangle distritization we
