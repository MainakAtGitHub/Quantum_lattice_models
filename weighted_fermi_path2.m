function p=weighted_fermi_path2(inputfile,all,band)
if nargin < 1
...
klist = [KX(:),KY(:)];
[eigv,eigvc]=bands_precalc_klist2(inputfile,klist);
delta_max=0;
figure();
% colorlist = [1,0,0; 0,0.502,0.502; 0.5020,0.5020,0; 1,0,1; 0,1,1];
colorlist = [1,0,0; 0,1,0; 0,0,1; 1,0,1; 0,1,1]; %r g b magenta cyan
for j=1:nOrbitals
    ...
    if size(fermi_k_path_dummy,1)> 0
        ...
        [~,kSpaceEigenVectorsNormal]=bands_precalc_klist2(inputfile,fermi_k_path{j}(:,:));
...
%         scatter3(kx1,ky1,w1,'MarkerEdgeColor',[0 0.75 0.75],'MarkerFaceColor',[0 .75 .75]);
        ...
        scatter3(fermi_k_path{j}(:,1)/pi,fermi_k_path{j}(:,2)/pi,weight,5,orb_color,'filled');
%         legend(orb_color(ii,:));
%         colorbar;
    ...
    xlabel('k_x/\pi');
    ylabel('k_y/\pi');
    title(['weighted fermi surface',', n=',num2str(n0),', kT=',num2str(kT)]);
    testgca=gca;
    testgca.FontSize=30;
    box on;
end
